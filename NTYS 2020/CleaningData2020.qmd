...
```{r setup}
getwd()
YTSData <- read.csv("OG_nyts2020.csv")
```

```{r renamecol}
#first select columns to keep
YTSData <- YTSData %>% 
  select("QN2", "QN3", "QN6", "QN8", "QN9", "QN17", "QN18", "QN19", "QN22", "QN25", "QN26", "QN31", "QN32", "QN33", "QN82", "QN61", "QN63", "QN93", "QN102", "QN98")
...
YTSData <- YTSData %>% rename(
  ...
  "Tobacco_Promotions_Internet" = QN98
)
```

```{r Duplicating and saving new dataset}
YTSDataNumbers <- YTSData
write.csv(YTSDataNumbers, file = "NYTS2020DataNumbers.csv")
```
...
```{r move Dataset}
write.csv(YTSData, file = "NYTSData2020.csv")
```
